...
def change(move_xo, queue) -> int or False:
...
def computer(moveX) -> None:
    lst_bool = [x['state'] == NORMAL for x in lst]
    lst_ind = [g for g, t in enumerate(lst_bool) if t is True]
    if len(lst_ind) != 0:
        if moveO[0] is not False and lst_bool[moveO[0]] is True:
            move = moveO[0]
        elif moveX is not False and lst_bool[moveX] is True:
            move = moveX
        else:
            move = choice(lst_ind)
        lst[move].config(text=sim[0], state=DISABLED)
        moveO[0] = change(move, 2)
        check()
